                    ...
                    <div class="row">
                        <div class="col-12">
                            <form action="{{ route('discussions.store') }}" method="POST">
                                    ...
                                    <select class="tags-select form-select @error('tag_slug') is-invalid @enderror" name="tag_slug[]" id="tag_slug" multiple="multiple">
                                        @foreach ($tags as $tag)
                                            <option value="{{ $tag->slug }}"
                                                @if (is_array(old('tag_slug')) && in_array($tag->slug, old('tag_slug'))) {{ 'selected' }} @endif>{{ $tag->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('tag_slug')
                                        <div class="invalid-feedback">
                                ...
                                <div class="mb-3">
                                    <label for="content" class="form-label @error('content') is-invalid @enderror">Quetions</label>
